...
const DepartmentList = memo(({ departments, onEdit, onDelete }) => {
  const columns = [
    { key: 'name', label: 'Name' },
    { key: 'shortName', label: 'Short Name' },
    {
      ...
      render: (value, item) => {
        if (item.managerName) {
          const fullName = `${item.managerName.firstName || ''} ${item.managerName.lastName || ''}`.trim();
          return fullName || item.managerName.shortName || 'N/A';
        }
        return 'N/A';
      },
    },
