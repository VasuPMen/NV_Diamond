...
const EmployeeList = memo(({ employees, onEdit, onDelete }) => {
    const columns = [
        {
            key: 'name',
            label: 'Name',
        ...
        { key: 'shortName', label: 'Short Name' },
        {
            key: 'manager',
            label: 'Manager',
            render: (value, item) => item.manager ? `${item.manager.firstName || ''} ${item.manager.lastName || ''}`.trim() : '-'
        },
        { key: 'mobileNo', label: 'Mobile No' },
        { key: 'emailId', label: 'Email' },
        { key: 'workingType', label: 'Working Type' },
    ...
    return (
        <Table
            ...
            emptyDescription="Get started by creating a new employee"
        />
    );
});
